...
export default function WatchListDashboard() {
  ...
  function removeTicker(ticker) {
    setConfirmDeleteModal({ open: true, ticker });
  }

  function archiveTicker(ticker) {
    setArchiveModal({ open: true, ticker, reason: "" });
  }

  function restoreTicker(ticker) {
    setRows((prev) => prev.map((row) => (row.ticker === ticker ? { ...row, archived: false } : row)));
  }
  ...
  return (
    <div dir="rtl" className={`min-h-screen p-4 lg:p-6 ${theme.app}`}>
                    ...
                    return (
                      <React.Fragment key={row.ticker}>
                        <tr onClick={() => setDrawerTicker(isOpen ? "" : row.ticker)} className={`cursor-pointer border-t transition ${theme.row} ${isOpen ? "bg-cyan-500/10 ring-1 ring-cyan-500/30" : row.alert.state === "NEAR" ? "bg-yellow-500/10 ring-1 ring-yellow-500/40" : row.isReadyToTrade ? "bg-emerald-500/10 ring-1 ring-emerald-500/40" : row.alert.triggered ? "ring-1 ring-emerald-500/40" : ""}`}>
                          <td className="p-4"><div className="flex items-center gap-2"><span title={row.alert.label} className={`h-3 w-3 rounded-full ${row.alert.dot || "bg-slate-500"} ${row.alert.state === "NEAR" ? "animate-pulse scale-125" : ""}`} /><button onClick={(e) => { e.stopPropagation(); openChart(row.ticker); }} className={`font-black hover:underline ${theme.strong}`}>{row.ticker} ↗</button></div></td>
                          ...
                          <td className="p-4"><div className="flex gap-2"><Button onClick={(e) => { e.stopPropagation(); setDrawerTicker(isOpen ? "" : row.ticker); }} className="border border-blue-500/30 text-blue-300">מגירה</Button><Button onClick={(e) => { e.stopPropagation(); setConfirmAiModal({ open: true, ticker: row.ticker }); }} className="border border-emerald-500/30 text-emerald-300">אשר</Button><Button onClick={(e) => { e.stopPropagation(); archiveTicker(row.ticker); }} className="border border-yellow-500/30 text-yellow-300">ארכיון</Button><Button onClick={(e) => { e.stopPropagation(); removeTicker(row.ticker); }} className="border border-red-500/30 text-red-300">מחק</Button></div></td>
                        </tr>

                        {isOpen && (
                          <tr className="border-t border-cyan-500/20 bg-[#07111f]/90">
                            <td colSpan={15} className="p-0">
                              <div className="border-t border-cyan-500/20 bg-gradient-to-l from-cyan-500/10 via-slate-950/20 to-transparent px-5 py-5">
                                <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                                  <div>
                                    <div className="flex items-center gap-3"><span className="rounded-2xl border border-cyan-500/30 bg-cyan-500/10 px-4 py-2 text-2xl font-black text-white">{row.ticker}</span><span className={`rounded-full px-3 py-1 text-xs font-black ${priority.className}`}>{priority.text}</span><span className={`rounded-full border px-3 py-1 text-xs font-black ${row.alert.className}`}>{row.alert.label}</span></div>
                                    <p className={`mt-2 text-xs ${theme.muted}`}>מגירת עבודה — תזה, אלרט, מבנה, אזור כניסה ופסילת טרייד.</p>
                                  </div>
                                  <div className="flex flex-wrap gap-2"><Button onClick={() => openChart(row.ticker)} className="border border-cyan-500/30 bg-cyan-500/10 text-cyan-300">פתח גרף ↗</Button><Button onClick={() => setConfirmAiModal({ open: true, ticker: row.ticker })} className="border border-emerald-500/30 bg-emerald-500/10 text-emerald-300">אשר AI</Button><Button onClick={() => setDrawerTicker("")} className={theme.accent}>סגור</Button></div>
                                </div>

                                <div className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr_360px]">
                                  <div className="space-y-4">
                                    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><div className={`rounded-2xl border p-4 ${theme.soft} ${theme.border}`}><div className={`text-xs font-black ${theme.muted}`}>Priority</div><div className="mt-2 text-3xl font-black text-white">{row.priority}</div><div className={`mt-1 text-xs ${theme.muted}`}>מתוך 100</div></div><div className={`rounded-2xl border p-4 ${theme.soft} ${theme.border}`}><div className={`text-xs font-black ${theme.muted}`}>AI Status</div><div className="mt-3"><span className={`rounded-xl border px-3 py-2 text-sm font-black ${statusClass(a.aiStatus)}`}>{a.aiStatus}</span></div></div><div className={`rounded-2xl border p-4 ${theme.soft} ${theme.border}`}><div className={`text-xs font-black ${theme.muted}`}>Entry Zone</div><div className="mt-2 text-lg font-black text-emerald-300">{a.entryZone}</div></div><div className={`rounded-2xl border p-4 ${theme.soft} ${theme.border}`}><div className={`text-xs font-black ${theme.muted}`}>Invalidation</div><div className="mt-2 text-lg font-black text-red-300">{a.invalidation}</div></div></div>
                                    <div className={`rounded-3xl border p-4 ${theme.soft} ${theme.border}`}><div className="mb-3 flex items-center justify-between"><h4 className={`text-base font-black ${theme.strong}`}>מבנה והחלטת טרייד</h4><span className={`rounded-xl border px-3 py-1 text-xs font-black ${setupClass(a.setup)}`}>{a.setup}</span></div><div className="grid gap-3 lg:grid-cols-3"><div className="rounded-2xl border border-slate-700/70 bg-black/20 p-3"><div className={`text-xs ${theme.muted}`}>Structure</div><div className="mt-2 text-sm font-bold text-slate-100">{a.structure}</div></div><div className="rounded-2xl border border-slate-700/70 bg-black/20 p-3"><div className={`text-xs ${theme.muted}`}>Why</div><div className="mt-2 text-sm text-slate-300">{a.why}</div></div><div className="rounded-2xl border border-slate-700/70 bg-black/20 p-3"><div className={`text-xs ${theme.muted}`}>Score</div><div className="mt-3 flex items-center gap-3"><div className="h-2 flex-1 rounded-full bg-slate-700"><div className="h-2 rounded-full bg-cyan-300" style={{ width: `${a.score}%` }} /></div><b className="text-white">{a.score}</b></div></div></div></div>
                                    <div className={`rounded-3xl border p-4 ${theme.soft} ${theme.border}`}><h4 className={`mb-3 text-base font-black ${theme.strong}`}>תזה אישית</h4><div className="grid gap-3 lg:grid-cols-[180px_1fr]"><div><label className={`mb-2 block text-sm font-black ${theme.muted}`}>תאריך תזה</label><input type="date" value={row.thesisDate || today} onChange={(e) => updateRow(row.ticker, { thesisDate: e.target.value })} className={`w-full rounded-2xl border p-3 text-sm outline-none ${theme.input}`} /></div><div><label className={`mb-2 block text-sm font-black ${theme.muted}`}>כותרת קצרה</label><select value={row.thesisTitle || ""} onChange={(e) => updateRow(row.ticker, { thesisTitle: e.target.value })} className={`w-full rounded-2xl border p-3 text-sm outline-none ${theme.input}`}><option value="">בחר תבנית</option><option value="Spike + Wait Base">Spike + Wait Base</option><option value="Pullback Continuation">Pullback Continuation</option><option value="Base Before Breakout">Base Before Breakout</option><option value="Breakout Setup">Breakout Setup</option><option value="Range / Accumulation">Range / Accumulation</option><option value="Weak / Avoid">Weak / Avoid</option></select></div></div><div className="mt-3"><label className={`mb-2 block text-sm font-black ${theme.muted}`}>מה ראיתי / תוכנית פעולה</label><textarea value={row.thesis || ""} onChange={(e) => updateRow(row.ticker, { thesis: e.target.value })} placeholder="לדוגמה: מחכה לפריצה מעל אלרט, לא נכנס לפני base, פסילה מתחת לנר מבני." className={`min-h-[150px] w-full rounded-2xl border p-3 text-sm outline-none ${theme.input}`} /></div></div>
                                  </div>
                                  <div className="space-y-4"><div className={`rounded-3xl border p-4 ${theme.soft} ${theme.border}`}><h4 className={`mb-3 text-base font-black ${theme.strong}`}>אלרט ובדיקה</h4><div className="space-y-3"><div><label className={`mb-2 block text-sm font-black ${theme.muted}`}>Alert Price</label><input type="number" value={row.alertPrice || ""} onChange={(e) => updateRow(row.ticker, { alertPrice: e.target.value })} placeholder="לדוגמה 8.50" className={`w-full rounded-2xl border p-3 text-sm outline-none ${theme.input}`} /></div><div><label className={`mb-2 block text-sm font-black ${theme.muted}`}>סוג אלרט</label><select value={row.alertType || "above"} onChange={(e) => updateRow(row.ticker, { alertType: e.target.value })} className={`w-full rounded-2xl border p-3 text-sm outline-none ${theme.input}`}><option value="above">Above — מעל מחיר</option><option value="below">Below — מתחת מחיר</option></select></div><div className={`rounded-2xl border p-3 ${row.alert.className}`}><div className="text-xs font-black">מצב אלרט</div><div className="mt-1 text-lg font-black">{row.alert.label}</div></div><Button onClick={async () => { setLoading(true); setLastError(""); try { const updated = await loadTicker(row); setRows((prev) => prev.map((item) => (item.ticker === row.ticker ? updated : item))); setLastRefreshTime(new Date().toLocaleTimeString()); } catch { setLastError("בדיקת מניה נכשלה. בדוק API Key / חיבור / טיקר."); } finally { setLoading(false); } }} className="w-full border border-emerald-500/30 bg-emerald-500/10 text-emerald-300">{loading ? "טוען..." : "טען מניה"}</Button></div></div><div className={`rounded-3xl border p-4 ${theme.soft} ${theme.border}`}><h4 className={`mb-3 text-base font-black ${theme.strong}`}>ניהול מהיר</h4><div className="grid gap-2"><Button onClick={() => setConfirmAiModal({ open: true, ticker: row.ticker })} className="border border-emerald-500/30 text-emerald-300">אשר סטאפ AI</Button><Button onClick={() => archiveTicker(row.ticker)} className="border border-yellow-500/30 text-yellow-300">העבר לארכיון</Button><Button onClick={() => removeTicker(row.ticker)} className="border border-red-500/30 text-red-300">מחק מהרשימה</Button></div></div></div>
      ...
      {confirmDeleteModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className={`w-full max-w-md rounded-3xl border p-6 ${theme.card}`}>
            ...
            <p className={`mt-2 text-sm ${theme.muted}`}>האם למחוק את {confirmDeleteModal.ticker} מהרשימה?</p>
            <div className="mt-5 flex gap-3"><Button onClick={() => { setRows((prev) => prev.filter((row) => row.ticker !== confirmDeleteModal.ticker)); setConfirmDeleteModal({ open: false, ticker: null }); }} className="border border-red-500/30 bg-red-500/10 text-red-300">מחק</Button><Button onClick={() => setConfirmDeleteModal({ open: false, ticker: null })} className={theme.navIdle}>ביטול</Button></div>
          </div>
        </div>
      )}
      ...
      {archiveModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className={`w-full max-w-lg rounded-3xl border p-6 ${theme.card}`}>
            ...
            <textarea value={archiveModal.reason} onChange={(e) => setArchiveModal((prev) => ({ ...prev, reason: e.target.value }))} placeholder="למה המניה עוברת לארכיון?" className={`mt-4 min-h-[120px] w-full rounded-2xl border p-3 ${theme.input}`} />
            <div className="mt-5 flex gap-3"><Button onClick={() => { setRows((prev) => prev.map((row) => row.ticker === archiveModal.ticker ? { ...row, archived: true, archiveReason: archiveModal.reason } : row)); setArchiveModal({ open: false, ticker: null, reason: "" }); }} className="border border-yellow-500/30 bg-yellow-500/10 text-yellow-300">העבר</Button><Button onClick={() => setArchiveModal({ open: false, ticker: null, reason: "" })} className={theme.navIdle}>ביטול</Button></div>
          </div>
        </div>
      )}
      ...
      {confirmAiModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className={`w-full max-w-md rounded-3xl border p-6 ${theme.card}`}>
            <h3 className={`text-xl font-black ${theme.strong}`}>אישור סטאפ AI</h3>
            <p className={`mt-2 text-sm ${theme.muted}`}>לאשר את {confirmAiModal.ticker} כ־READY?</p>
            <div className="mt-5 flex gap-3"><Button onClick={() => { setRows((prev) => prev.map((row) => row.ticker === confirmAiModal.ticker ? { ...row, analysis: { ...(row.analysis || emptyAnalysis()), aiStatus: "READY" } } : row)); setConfirmAiModal({ open: false, ticker: null }); }} className="border border-emerald-500/30 bg-emerald-500/10 text-emerald-300">אשר</Button><Button onClick={() => setConfirmAiModal({ open: false, ticker: null })} className={theme.navIdle}>ביטול</Button></div>
          </div>
        </div>
      )}
    </div>
