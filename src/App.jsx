  ...
  return (
    <div dir="rtl" className={`min-h-screen p-4 lg:p-6 ${theme.app}`}>
      <button
        ...
        className="fixed bottom-6 left-6 z-40 rounded-full border border-slate-700 bg-[#111827] px-4 py-2 text-sm font-black text-white shadow-xl hover:bg-slate-800"
      >
        ⚙️ הגדרות
      </button>
      ...
      <div className="mx-auto w-full max-w-[96vw] space-y-5">
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <span className={`rounded-full border px-3 py-1 text-xs font-black ${apiConnected ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-300" : "border-red-500/30 bg-red-500/10 text-red-300"}`}>
                  {apiConnected ? "🟢 API מחובר" : "🔴 API לא מחובר"}
                </span>
                <span className={`rounded-full px-3 py-1 text-xs font-black ${theme.navIdle}`}>
                  עדכון אחרון: {lastRefreshTime || "—"}
                </span>
              </div>
            </div>
        ...
        <section className={`rounded-3xl border p-4 ${theme.card}`}>
          <div className="grid gap-3 lg:grid-cols-2">
            <input
              ...
              className={`rounded-2xl border px-4 py-3 text-sm outline-none ${theme.input}`}
            />
            ...
            <Button onClick={loadAllLive} disabled={loading} className="border border-emerald-500/30 bg-emerald-500/10 text-emerald-300 disabled:opacity-50">
        ...
        {activePanel === "main" && (
          <section className={`rounded-3xl border p-3 ${theme.card}`}>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button onClick={() => setFocusMode(!focusMode)} className={`rounded-full px-3 py-2 text-xs font-black ${focusMode ? "bg-emerald-500 text-black" : "bg-slate-800 text-slate-300"}`}>
                🎯 מצב פוקוס
              </button>
            </div>
          </section>
          ...
          <section className={`rounded-3xl border p-5 ${theme.card}`}>
            ...
            <div className="grid gap-3 xl:grid-cols-2 2xl:grid-cols-3">
              {topPriorityRows.map(row => {
                ...
                return (
                  <div key={row.ticker} className={`rounded-3xl border p-4 ${theme.soft} ${theme.border}`}>
                ...
                <tbody>
                  {playbook.map(item => (
                    <tr key={item.mode} className={`border-b ${theme.row}`}>
                      ...
                      <td className="p-3 text-emerald-300">{item.traderPosition}</td>
        ...
        {activePanel === "archive" && (
          <section className={`rounded-3xl border p-5 ${theme.card}`}>
            ...
            {archivedRows.length === 0 ? (
              <div className={`rounded-2xl border p-4 text-sm ${theme.border} ${theme.soft} ${theme.muted}`}>אין כרגע מניות בארכיון.</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full min-w-[900px] text-right text-sm">
                  <thead className={`border-b ${theme.border} ${theme.muted}`}>
                    <tr>
                      <th className="p-3">טיקר</th>
                      <th className="p-3">אלרט</th>
                      <th className="p-3">סטאפ</th>
                  ...
                  <tbody>
                    {archivedRows.map(row => (
                      <tr key={row.ticker} className={`border-b ${theme.row}`}>
                        ...
                        <td className="p-3"><div className="flex gap-2"><Button onClick={() => restoreTicker(row.ticker)} className="border border-emerald-500/30 text-emerald-300">החזר</Button><Button onClick={() => removeTicker(row.ticker)} className="border border-red-500/30 text-red-300">מחק לצמיתות</Button></div></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {activePanel === "main" && selectedRow && (
          <section className={`rounded-3xl border p-5 ${theme.card}`}>
            ...
            <div className={`mb-4 rounded-3xl border p-4 ${theme.soft} ${theme.border}`}>
              ...
              <div className={`mt-3 grid gap-2 text-sm lg:grid-cols-4 ${theme.muted}`}>
                <div>אלרט: <b>{selectedRow.alert.label}</b></div>
                <div>דירוג: <b>{selectedRow.rating || 0}⭐</b></div>
                <div>AI: <b>{selectedRow.analysis.aiStatus}</b></div>
                <div>סטאפ: <b>{selectedRow.analysis.setup}</b></div>
              </div>
              <p className={`mt-3 text-xs ${theme.muted}`}>ה־Priority מחבר אלרט, דירוג אישי, סטטוס AI, סוג Setup ו־Score.</p>
            </div>

            <div className="mb-3 rounded-2xl border border-amber-500/30 bg-amber-500/10 p-3 text-sm font-bold text-amber-200">
              📌 הוראות: תכניס Alert Price ואז לחץ על "טען מניה" → נטען דאטה חי עבור המניה הזאת בלבד והאלרט מופעל.
            </div>
              ...
              <div>
                <div className="mb-3 grid gap-3 lg:grid-cols-[180px_1fr_160px]">
                  <div>
                    ...
                    <input type="number" value={selectedRow.alertPrice || ""} onChange={e => updateRow(selectedRow.ticker, { alertPrice: e.target.value })} placeholder="לדוגמה 8.50" className={`w-full rounded-2xl border p-3 text-sm outline-none ${theme.input}`} />
                  </div>
                  <div>
                    <label className={`mb-2 block text-sm font-black ${theme.muted}`}>סוג אלרט</label>
                    <select value={selectedRow.alertType || "above"} onChange={e => updateRow(selectedRow.ticker, { alertType: e.target.value })} className={`w-full rounded-2xl border p-3 text-sm outline-none ${theme.input}`}>
                  ...
                  <div>
                    ...
                    <span className={`inline-flex w-full items-center justify-center rounded-2xl border p-3 text-sm font-black ${selectedRow.alert.className}`}>{selectedRow.alert.label}</span>
                    <Button
                      onClick={async () => {
                        setLoading(true);
                        setLastError("");
                      ...
                      className="mt-2 w-full border border-emerald-500/30 bg-emerald-500/10 text-emerald-300"
                    >
                ...
                <div className="mb-3 grid gap-3 lg:grid-cols-[180px_1fr]">
                  <div>
                    ...
                    <input type="date" value={selectedRow.thesisDate || today} onChange={e => updateRow(selectedRow.ticker, { thesisDate: e.target.value })} className={`w-full rounded-2xl border p-3 text-sm outline-none ${theme.input}`} />
                  </div>
                  <div>
                    <label className={`mb-2 block text-sm font-black ${theme.muted}`}>כותרת קצרה</label>
                    <select value={selectedRow.thesisTitle || ""} onChange={e => updateRow(selectedRow.ticker, { thesisTitle: e.target.value })} className={`w-full rounded-2xl border p-3 text-sm outline-none ${theme.input}`}>
                ...
                <textarea value={selectedRow.thesis || ""} onChange={e => updateRow(selectedRow.ticker, { thesis: e.target.value })} placeholder="לדוגמה: מחכה לפריצה מעל אלרט, לא נכנס לפני base." className={`min-h-[180px] w-full rounded-2xl border p-3 text-sm outline-none ${theme.input}`} />
              </div>

              <div>
                <label className={`mb-2 block text-sm font-black ${theme.muted}`}>תמונת גרף</label>
                <input type="file" accept="image/*" onChange={e => handleImageUpload(selectedRow.ticker, e.target.files && e.target.files[0])} className={`w-full rounded-2xl border p-3 text-sm ${theme.input}`} />
                {selectedRow.chartImage ? (
                  ...
                  <div className={`mt-3 rounded-2xl border p-4 text-sm ${theme.border} ${theme.soft} ${theme.muted}`}>עדיין לא הועלתה תמונת גרף.</div>
                )}
              </div>
            </div>
          </section>
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[1180px] text-right text-xs xl:text-sm">
                <thead className={`${theme.head} text-xs uppercase tracking-wider`}>
                  <tr>
                    <th className="p-4">טיקר</th>
                    <th className="p-4">מחיר</th>
                    <th className="p-4">1D%</th>
                    <th className="p-4">עדיפות</th>
                    <th className="p-4">דירוג</th>
                    <th className="p-4">מצב אלרט</th>
                    <th className="p-4">סטטוס שלך</th>
                    <th className="p-4">AI</th>
                    <th className="p-4">ציון</th>
                    <th className="p-4">סטאפ</th>
                    <th className="p-4">מבנה</th>
                    <th className="p-4">כניסה</th>
                    <th className="p-4">פסילה</th>
                    <th className="p-4">למה</th>
                ...
                <tbody>
                  {visibleRows.map(row => {
                    ...
                    return (
                      <tr key={row.ticker} className={`border-t ${theme.row} ${row.alert.state === "NEAR" ? "bg-yellow-500/10 ring-1 ring-yellow-500/40" : row.isReadyToTrade ? "bg-emerald-500/10 ring-1 ring-emerald-500/40" : row.alert.triggered ? "ring-1 ring-emerald-500/40" : ""}`}>
                        <td className="p-4">
                          <div className="flex items-center gap-2">
                            <span title={row.alert.label} className={`h-3 w-3 rounded-full ${row.alert.dot || "bg-slate-500"} ${row.alert.state === "NEAR" ? "animate-pulse scale-125" : ""}`} />
                        ...
                        <td className="p-4">
                          <select value={row.rating || 0} onChange={e => updateRow(row.ticker, { rating: Number(e.target.value) })} className={`rounded-lg border px-2 py-1 font-bold ${theme.input}`}>
                        ...
                        <td className="p-4">
                          <div className="flex gap-2">
                            <Button onClick={() => setDrawerTicker(row.ticker)} className="border border-blue-500/30 text-blue-300">מגירה</Button>
                            <Button onClick={() => setConfirmAiModal({ open: true, ticker: row.ticker })} className="border border-emerald-500/30 text-emerald-300">אשר</Button>
                            <Button onClick={() => archiveTicker(row.ticker)} className="border border-yellow-500/30 text-yellow-300">ארכיון</Button>
                            <Button onClick={() => removeTicker(row.ticker)} className="border border-red-500/30 text-red-300">מחק</Button>
                          </div>
                        </td>
                      </tr>
                    );
        ...
        <div className={`rounded-2xl border p-3 text-xs ${theme.card} ${theme.muted}`}>
          Smart Alert: אם המחיר עבר את היעד — השורה תעלה למעלה ותסומן. אם המחיר קרוב עד 3% — תקבל סימון קרוב.
        </div>
      </div>

      {showSettings && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 p-4" onClick={() => setShowSettings(false)}>
          <section className={`w-full max-w-6xl rounded-3xl border p-5 ${theme.card}`} onClick={event => event.stopPropagation()}>
            <div className="mb-5 flex items-center justify-between">
              ...
              <button onClick={() => setShowSettings(false)} className="rounded-full border border-slate-700 bg-slate-900 px-4 py-2 text-sm font-black text-white hover:bg-slate-800">סגור</button>
            </div>
              ...
              <div className="rounded-2xl border border-slate-700 bg-slate-900/80 p-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[["sm", "קטן"], ["md", "רגיל"], ["lg", "גדול"]].map(([value, label]) => (
                    <button key={value} onClick={() => setButtonSize(value)} className={`rounded-full px-3 py-2 text-xs font-black transition ${buttonSize === value ? "bg-white text-slate-950" : "bg-slate-800 text-slate-300 hover:bg-slate-700"}`}>{label}</button>
                  ))}
                </div>
              </div>
              <div className="rounded-2xl border border-slate-700 bg-slate-900/80 p-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[["none", "שטוח"], ["normal", "עדין"], ["strong", "חזק"]].map(([value, label]) => (
                    <button key={value} onClick={() => setButtonShadow(value)} className={`rounded-full px-3 py-2 text-xs font-black transition ${buttonShadow === value ? "bg-white text-slate-950" : "bg-slate-800 text-slate-300 hover:bg-slate-700"}`}>{label}</button>
                  ))}
                </div>
              </div>
              <div className="rounded-2xl border border-slate-700 bg-slate-900/80 p-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[["rounded-xl", "מעוגל"], ["rounded-full", "עגול"], ["rounded-none", "חד"], ["btn-3d", "תלת ממד"]].map(([value, label]) => (
                    <button key={value} onClick={() => setButtonStyle(value)} className={`rounded-full px-3 py-2 text-xs font-black transition ${buttonStyle === value ? "bg-white text-slate-950" : "bg-slate-800 text-slate-300 hover:bg-slate-700"}`}>{label}</button>
                  ))}
                </div>
              </div>
              <div className="rounded-2xl border border-slate-700 bg-slate-900/80 p-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {Object.entries(themes).map(([key, item]) => (
                    <button key={key} onClick={() => setThemeName(key)} className={`rounded-full px-3 py-2 text-xs font-black transition ${themeName === key ? "bg-white text-slate-950" : "bg-slate-800 text-slate-300 hover:bg-slate-700"}`}>{item.name}</button>
                  ))}
                </div>
              </div>
            </div>
      ...
      {confirmAiModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className={`w-[420px] rounded-3xl border p-5 ${theme.card}`}>
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button onClick={() => setConfirmAiModal({ open: false, ticker: null })} className="rounded-xl border border-slate-700 px-3 py-2 text-sm">בטל</button>
              <button onClick={() => { setRows(prev => prev.map(row => row.ticker === confirmAiModal.ticker ? { ...row, userStatus: row.analysis?.aiStatus || "WATCH" } : row)); setConfirmAiModal({ open: false, ticker: null }); }} className="rounded-xl border border-emerald-500/30 px-3 py-2 text-sm font-bold text-emerald-300">כן, מאשר</button>
            </div>
          </div>
        </div>
      )}
      ...
      {confirmDeleteModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className={`w-[420px] rounded-3xl border p-5 ${theme.card}`}>
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button onClick={() => setConfirmDeleteModal({ open: false, ticker: null })} className="rounded-xl border border-slate-700 px-3 py-2 text-sm">בטל</button>
              <button onClick={() => { setRows(prev => prev.filter(row => row.ticker !== confirmDeleteModal.ticker)); setConfirmDeleteModal({ open: false, ticker: null }); }} className="rounded-xl border border-red-500/30 px-3 py-2 text-sm font-bold text-red-300">כן, מחק</button>
            </div>
          </div>
        </div>
      )}
      ...
      {archiveModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className={`w-[420px] rounded-3xl border p-5 ${theme.card}`}>
            ...
            <textarea value={archiveModal.reason} onChange={e => setArchiveModal(prev => ({ ...prev, reason: e.target.value }))} placeholder="לא חובה אבל מומלץ" className={`mt-3 w-full rounded-2xl border p-3 text-sm ${theme.input}`} />
            <div className="mt-4 flex justify-end gap-2">
              <button onClick={() => setArchiveModal({ open: false, ticker: null, reason: "" })} className="rounded-xl border border-slate-700 px-3 py-2 text-sm">בטל</button>
              <button onClick={() => { setRows(prev => prev.map(row => row.ticker === archiveModal.ticker ? { ...row, archived: true, archiveReason: archiveModal.reason || "" } : row)); setArchiveModal({ open: false, ticker: null, reason: "" }); }} className="rounded-xl border border-yellow-500/30 px-3 py-2 text-sm font-bold text-yellow-300">אשר ארכיון</button>
            </div>
          </div>
        </div>
      )}
