...
const themes = {
  black: {
    name: "שחור",
    app: "bg-[#050505] text-zinc-100",
    card: "bg-zinc-950 border-zinc-800",
    soft: "bg-zinc-900",
    input: "bg-black border-zinc-700 text-zinc-100 placeholder:text-zinc-500",
    muted: "text-zinc-400",
    strong: "text-white",
    head: "bg-black text-zinc-500",
    row: "border-zinc-800 hover:bg-zinc-900/70",
    navIdle: "bg-black text-zinc-300 hover:bg-zinc-900 border border-zinc-800",
...
const playbook = [
  {
    mode: "Spike",
    check: "ווליום חריג + נר חזק",
    result: "התחלת מהלך",
    action: "WATCH / READY אם יש המשכיות",
    traderPosition: "לא כניסה מיידית — סימון למעקב",
    focus: "לחכות ל־base קטן אחרי הספייק",
    avoid: "לא לרדוף אחרי גאפ פתיחה",
  },
  {
    mode: "Pullback",
    check: "ירידה במחיר + ירידה בווליום",
    result: "תיקון בריא",
    action: "READY",
    traderPosition: "עמדת כניסה אפשרית אחרי התייצבות",
    focus: "לחפש Higher Low / נר עצירה / טווח קטן",
    avoid: "לא לקנות אם התיקון נהיה חד",
  },
  {
    mode: "Breakdown",
    check: "ירידה במחיר + ווליום גבוה",
    result: "מוכרים שולטים",
    action: "AVOID",
    traderPosition: "אין כניסה — הגנה על הון",
    focus: "לבדוק אם נשבר range low או support",
    avoid: "להימנע עד reclaim ברור",
  },
  {
    mode: "Base",
    check: "טווח צר + ווליום יורד",
    result: "מתבשל מהלך",
    action: "WATCH",
    traderPosition: "עמדת המתנה — לפני טריגר",
    focus: "לסמן range high / range low",
    avoid: "להימנע אם הבסיס נשבר למטה",
  },
  {
    mode: "Breakout",
    check: "פריצה + ווליום עולה",
    result: "פריצה עם ביקוש",
    action: "READY",
    traderPosition: "כניסה רק אם יש אישור ולא גאפ פראי",
    focus: "לבדוק שהפריצה לא מתוחה מדי",
    avoid: "להימנע מפריצה בלי ווליום",
  },
];
...
function setupClass(setup) {
  ...
  if (setup === "Live Quote") return "text-slate-700 bg-slate-100 border-slate-300";
...
function analyzeAlert(row, price) {
  ...
  if (!target || !current) {
    return {
      ...
      label: "אין אלרט",
      dot: "bg-zinc-300",
      className: "text-zinc-600 bg-zinc-100 border-zinc-300",
    };
  }
  ...
  if (triggered) {
    return {
      ...
      label: "מוכנה",
      dot: "bg-emerald-500",
      className: "text-emerald-800 bg-emerald-100 border-emerald-400",
    };
  }
  ...
  if (absDistance <= 3) {
    return {
      state: "NEAR",
      triggered: false,
      label: `מתקרבת ${safeNum(absDistance, 1)}%`,
      dot: "bg-yellow-400",
  ...
  return {
    state: "WAIT",
    triggered: false,
    label: `ממתין ${safeNum(absDistance, 1)}%`,
    dot: "bg-zinc-400",
    className: "text-zinc-700 bg-zinc-100 border-zinc-300",
  };
}
...
export default function WatchListDashboard() {
  ...
  const viteFinnhubKey = typeof import.meta !== "undefined" && import.meta.env
    ? import.meta.env.VITE_FINNHUB_API_KEY
    : "";

  const [apiKey, setApiKey] = useState(
    () => localStorage.getItem("finnhub-key") || viteFinnhubKey || ""
  );

  const [themeName, setThemeName] = useState(() => localStorage.getItem(THEME_KEY) || "brown");
  ...
  const theme = themes[themeName] || themes.brown;
  ...
  const buttonDepth = buttonStyle === "btn-3d"
    ? `rounded-xl shadow-[0_5px_0_rgba(0,0,0,0.25)] active:translate-y-[3px] active:shadow-[0_2px_0_rgba(0,0,0,0.25)] transition-all ${sizeClass}`
    : `${buttonStyle} ${sizeClass} ${shadowClass}`;
  ...
  useEffect(() => {
    localStorage.setItem("finnhub-key", apiKey || "");
  }, [apiKey]);
  ...
  useEffect(() => {
    localStorage.setItem("telegram-bot-token", telegramBotToken || "");
  }, [telegramBotToken]);

  useEffect(() => {
    localStorage.setItem("telegram-chat-id", telegramChatId || "");
  }, [telegramChatId]);
  ...
  const addTicker = () => {
    ...
    setRows(prev => {
      ...
      if (existing) {
        return prev.map(row => row.ticker === ticker ? { ...row, archived: false } : row);
      }
  ...
  async function loadTicker(row) {
    ...
    try {
      const url = `https://finnhub.io/api/v1/quote?symbol=${row.ticker}&token=${cleanKey}`;
  ...
  async function loadAllLive() {
    ...
    try {
      ...
      for (const row of rows) {
        updated.push(await loadTicker(row));
      }
      setRows(updated);
  ...
  const Button = ({ children, className = "", ...props }) => (
    <button {...props} className={`${buttonDepth} font-black transition ${className}`}>
      {children}
    </button>
  );
  ...
  return (
    <div dir="rtl" className={`min-h-screen p-4 lg:p-6 ${theme.app}`}>
      <button
        ...
        className="fixed bottom-6 left-6 z-40 rounded-full bg-slate-900 px-4 py-2 text-sm font-black text-white shadow-xl ring-1 ring-slate-300 hover:bg-slate-700"
      >
        ⚙️ הגדרות
      </button>
      ...
      <div className="mx-auto w-full max-w-[96vw] space-y-5">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <span className={`rounded-full px-3 py-1 text-xs font-black ${apiConnected ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}>
                  {apiConnected ? "🟢 API CONNECTED" : "🔴 API OFFLINE"}
                </span>
                <span className={`rounded-full px-3 py-1 text-xs font-black ${theme.navIdle}`}>
        ...
        <section className={`rounded-2xl border p-3 ${theme.card}`}>
          <div className="grid gap-3 lg:grid-cols-2">
            <input
              ...
              className={`rounded-xl border px-4 py-3 text-sm outline-none focus:border-yellow-500 ${theme.input}`}
            />
            <div className="flex gap-2">
              <input
                ...
                className={`w-full rounded-xl border px-4 py-3 text-sm uppercase outline-none focus:border-yellow-500 ${theme.input}`}
              />
              <Button onClick={addTicker} className={theme.accent}>הוסף</Button>
            </div>
            <Button onClick={loadAllLive} disabled={loading} className="border border-emerald-300 bg-emerald-50 text-emerald-700 disabled:opacity-50">
              {loading ? "טוען..." : "Load Live Data"}
            </Button>
            ...
            <input
              ...
              className={`rounded-xl border px-4 py-3 text-sm outline-none ${theme.input}`}
            />
            <input
        ...
        {activePanel === "main" && (
          <section className={`rounded-2xl border p-2 ${theme.card}`}>
            <div className="flex flex-wrap items-center gap-2">
              {[["ALL", "הכל"], ["READY_TO_TRADE", `READY TO TRADE (${readyToTradeCount})`], ["TRIGGERED", "מוכנות"], ["NEAR", "מתקרבות"], ["HIGH_RATING", "דירוג גבוה ⭐4+"]].map(([key, label]) => (
                <Button key={key} onClick={() => setWatchFilter(key)} className={watchFilter === key ? theme.accent : theme.navIdle}>{label}</Button>
              ))}
              <button onClick={() => setFocusMode(!focusMode)} className={`rounded-full px-3 py-2 text-xs font-black ${focusMode ? "bg-emerald-600 text-white" : "bg-zinc-200 text-zinc-700"}`}>
                🎯 Focus Mode
              </button>
            </div>
          </section>
        ...
        {activePanel === "guide" && (
          <section className={`rounded-2xl border p-4 ${theme.card}`}>
            <div className="mb-3 flex items-center justify-between gap-3">
              <h2 className={`text-xl font-black ${theme.accentText}`}>מדריך זיהוי Setup + Volume</h2>
              <Button onClick={() => setActivePanel("main")} className={theme.accent}>חזור למסך ראשי</Button>
            </div>
              ...
              <table className="w-full min-w-[1200px] text-right text-sm">
                <thead className={`border-b ${theme.border} ${theme.muted}`}>
                  <tr>
                    ...
                    <th className="p-3">פירוש</th>
                    <th className="p-3">תוצאה</th>
                    <th className="p-3">עמדת טריידר</th>
                    <th className="p-3">מה לשים לב</th>
                    <th className="p-3">ממה להימנע</th>
                  </tr>
                </thead>
                <tbody>
                  {playbook.map(item => (
                    <tr key={item.mode} className={`border-b ${theme.row}`}>
                      ...
                      <td className="p-3 font-bold text-yellow-700">{item.action}</td>
                      <td className="p-3 text-emerald-700">{item.traderPosition}</td>
                      <td className="p-3 text-cyan-700">{item.focus}</td>
                      <td className="p-3 text-red-700">{item.avoid}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <section className={`rounded-2xl border p-4 ${theme.card}`}>
            <div className="mb-3 flex items-center justify-between gap-3">
              <div>
                ...
                <p className={`mt-1 text-sm ${theme.muted}`}>מניות שהועברו לארכיון.</p>
              </div>
              <Button onClick={() => setActivePanel("main")} className={theme.accent}>חזור למסך ראשי</Button>
            </div>
            {archivedRows.length === 0 ? (
              <div className={`rounded-xl border p-4 text-sm ${theme.border} ${theme.soft} ${theme.muted}`}>אין כרגע מניות בארכיון.</div>
            ) : (
              <div className="overflow-x-auto">
                  ...
                  <tbody>
                    {archivedRows.map(row => (
                      <tr key={row.ticker} className={`border-b ${theme.row}`}>
                        ...
                        <td className="p-3"><div className="flex gap-2"><Button onClick={() => restoreTicker(row.ticker)} className="border border-emerald-300 text-emerald-700">החזר</Button><Button onClick={() => removeTicker(row.ticker)} className="border border-red-300 text-red-700">מחק לצמיתות</Button></div></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {activePanel === "main" && selectedRow && (
          <section className={`rounded-2xl border p-4 ${theme.card}`}>
            <div className="mb-3 flex items-center justify-between gap-3">
              <div>
                <h2 className={`text-xl font-black ${theme.strong}`}>מגירה — {selectedRow.ticker}</h2>
                <p className={`mt-1 text-sm ${theme.muted}`}>תזה אישית, תאריך, סטאפ ותמונת גרף.</p>
              </div>
              <Button onClick={() => setDrawerTicker("")} className={theme.accent}>סגור מגירה</Button>
            </div>

            <div className={`mb-4 rounded-2xl border p-4 shadow-sm ${theme.soft} ${theme.border}`}>
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <div className={`text-xs font-black ${theme.muted}`}>Priority Engine</div>
                  <div className={`mt-1 text-2xl font-black ${theme.strong}`}>{selectedRow.priority}/100</div>
                </div>
                <span className={`rounded-full px-4 py-2 text-xs font-black ${priorityLabel(selectedRow.priority || 0).className}`}>
                  {priorityLabel(selectedRow.priority || 0).text}
                </span>
              </div>
              <div className={`mt-3 grid gap-2 text-sm lg:grid-cols-4 ${theme.muted}`}>
                <div>Alert: <b>{selectedRow.alert.label}</b></div>
                <div>Rating: <b>{selectedRow.rating || 0}⭐</b></div>
                <div>AI: <b>{selectedRow.analysis.aiStatus}</b></div>
                <div>Setup: <b>{selectedRow.analysis.setup}</b></div>
              </div>
              <p className={`mt-3 text-xs ${theme.muted}`}>ה־Priority מחבר אלרט, דירוג אישי, סטטוס AI, סוג Setup ו־Score.</p>
            </div>

            <div className="mb-3 rounded-xl border border-amber-300 bg-amber-50 p-3 text-sm font-bold text-amber-800">
              📌 הוראות: תכניס Alert Price ואז לחץ על "טען מניה" → נטען LOAD LIVE DATA עבור המניה הזאת בלבד והאלרט מופעל.
            </div>

            <div className="grid gap-4 lg:grid-cols-[1fr_420px]">
              <div>
                <div className="mb-3 grid gap-3 lg:grid-cols-[180px_1fr_160px]">
                  <div>
                    <label className={`mb-2 block text-sm font-black ${theme.muted}`}>Alert Price</label>
                    <input type="number" value={selectedRow.alertPrice || ""} onChange={e => updateRow(selectedRow.ticker, { alertPrice: e.target.value })} placeholder="לדוגמה 8.50" className={`w-full rounded-xl border p-3 text-sm outline-none focus:border-yellow-500 ${theme.input}`} />
                  </div>
                  <div>
                    <label className={`mb-2 block text-sm font-black ${theme.muted}`}>סוג אלרט</label>
                    <select value={selectedRow.alertType || "above"} onChange={e => updateRow(selectedRow.ticker, { alertType: e.target.value })} className={`w-full rounded-xl border p-3 text-sm outline-none focus:border-yellow-500 ${theme.input}`}>
                      <option value="above">Above — מעל מחיר</option>
                      <option value="below">Below — מתחת מחיר</option>
                    </select>
                  </div>
                  <div>
                    <label className={`mb-2 block text-sm font-black ${theme.muted}`}>מצב אלרט</label>
                    <span className={`inline-flex w-full items-center justify-center rounded-xl border p-3 text-sm font-black ${selectedRow.alert.className}`}>{selectedRow.alert.label}</span>
                    <Button
                      onClick={async () => {
                        setLoading(true);
                        setLastError("");
                        try {
                          const updated = await loadTicker(selectedRow);
                          setRows(prev => prev.map(row => row.ticker === selectedRow.ticker ? updated : row));
                          setLastRefreshTime(new Date().toLocaleTimeString());
                        } catch {
                          setLastError("בדיקת מניה נכשלה. בדוק API Key / חיבור / טיקר.");
                        } finally {
                          setLoading(false);
                        }
                      }}
                      className="mt-2 w-full border border-emerald-300 bg-emerald-50 text-emerald-700"
                    >
                ...
                <div className="mb-3 grid gap-3 lg:grid-cols-[180px_1fr]">
                  <div>
                    <label className={`mb-2 block text-sm font-black ${theme.muted}`}>תאריך תזה</label>
                    <input type="date" value={selectedRow.thesisDate || today} onChange={e => updateRow(selectedRow.ticker, { thesisDate: e.target.value })} className={`w-full rounded-xl border p-3 text-sm outline-none focus:border-yellow-500 ${theme.input}`} />
                  </div>
                  <div>
                    <label className={`mb-2 block text-sm font-black ${theme.muted}`}>כותרת קצרה</label>
                    <select value={selectedRow.thesisTitle || ""} onChange={e => updateRow(selectedRow.ticker, { thesisTitle: e.target.value })} className={`w-full rounded-xl border p-3 text-sm outline-none focus:border-yellow-500 ${theme.input}`}>
                ...
                <label className={`mb-2 block text-sm font-black ${theme.muted}`}>תזה / מה ראיתי</label>
                <textarea value={selectedRow.thesis || ""} onChange={e => updateRow(selectedRow.ticker, { thesis: e.target.value })} placeholder="לדוגמה: מחכה לפריצה מעל אלרט, לא נכנס לפני base." className={`min-h-[180px] w-full rounded-xl border p-3 text-sm outline-none focus:border-yellow-500 ${theme.input}`} />
              </div>

              <div>
                <label className={`mb-2 block text-sm font-black ${theme.muted}`}>תמונת גרף</label>
                <input type="file" accept="image/*" onChange={e => handleImageUpload(selectedRow.ticker, e.target.files && e.target.files[0])} className={`w-full rounded-xl border p-3 text-sm ${theme.input}`} />
                {selectedRow.chartImage ? (
                  <div className="mt-3 overflow-hidden rounded-xl border border-zinc-300 bg-black">
                    <img src={selectedRow.chartImage} alt={`chart-${selectedRow.ticker}`} className="max-h-[280px] w-full object-contain" />
                  </div>
                ) : (
                  <div className={`mt-3 rounded-xl border p-4 text-sm ${theme.border} ${theme.soft} ${theme.muted}`}>עדיין לא הועלתה תמונת גרף.</div>
                )}
              </div>
            </div>
          </section>
        ...
        {activePanel === "main" && (
          <section className={`overflow-hidden rounded-2xl border shadow-lg ${theme.card}`}>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[1180px] text-right text-xs xl:text-sm">
                <thead className={`${theme.head} text-xs uppercase tracking-wider`}>
                  <tr>
                    <th className="p-3">Ticker</th>
                ...
                <tbody>
                  {visibleRows.map(row => {
                    ...
                    return (
                      <tr key={row.ticker} className={`border-t ${theme.row} ${row.alert.state === "NEAR" ? "bg-yellow-100 ring-2 ring-yellow-400" : row.isReadyToTrade ? "bg-emerald-100 ring-2 ring-emerald-500" : row.alert.triggered ? "ring-2 ring-emerald-400" : ""}`}>
                        <td className="p-3">
                          <div className="flex items-center gap-2">
                            <span title={row.alert.label} className={`h-3 w-3 rounded-full ${row.alert.dot || "bg-zinc-300"} ${row.alert.state === "NEAR" ? "animate-pulse scale-125" : ""}`} />
                            <button onClick={() => openChart(row.ticker)} className={`font-black hover:underline ${theme.strong}`}>{row.ticker} ↗</button>
                          </div>
                        </td>
                        <td className="p-3 font-bold">{safeNum(a.price)}</td>
                        <td className={`p-3 font-bold ${Number(a.change1) >= 0 ? "text-emerald-600" : "text-red-600"}`}>{safeNum(a.change1, 1)}%</td>
                        <td className="p-3"><span className={`rounded-lg px-2 py-1 text-xs font-black ${priority.className}`}>{row.priority}</span></td>
                        <td className="p-3">
                          <select value={row.rating || 0} onChange={e => updateRow(row.ticker, { rating: Number(e.target.value) })} className="rounded-lg border px-2 py-1 font-bold">
                        ...
                        <td className="p-3"><span className={`rounded-lg border px-2 py-1 font-black ${row.alert.className}`}>{row.alert.label}</span></td>
                        <td className="p-3">
                          <select value={row.userStatus || "WATCH"} onChange={e => updateRow(row.ticker, { userStatus: e.target.value })} className={`rounded-lg border px-2 py-1 font-bold ${statusClass(row.userStatus || "WATCH")}`}>
                            <option value="READY">READY</option>
                            <option value="WATCH">WATCH</option>
                            <option value="AVOID">AVOID</option>
                          </select>
                        </td>
                        <td className="p-3"><span className={`rounded-lg border px-2 py-1 font-black ${statusClass(a.aiStatus)}`}>{a.aiStatus}</span></td>
                        <td className="p-3"><div className="flex items-center gap-2"><div className="h-2 w-20 rounded-full bg-zinc-300"><div className="h-2 rounded-full bg-yellow-500" style={{ width: `${a.score}%` }} /></div><b>{a.score}</b></div></td>
                        <td className="p-3"><span className={`rounded-lg border px-3 py-1 font-black ${setupClass(a.setup)}`}>{a.setup}</span></td>
                        <td className={`p-3 ${theme.strong}`}>{a.structure}</td>
                        <td className="p-3 font-bold text-emerald-600">{a.entryZone}</td>
                        <td className="p-3 font-bold text-red-600">{a.invalidation}</td>
                        <td className={`max-w-[300px] p-3 ${theme.muted}`}>{a.why}</td>
                        <td className="p-3">
                          <div className="flex gap-2">
                            <Button onClick={() => setDrawerTicker(row.ticker)} className="border border-blue-300 text-blue-600">מגירה</Button>
                            <Button onClick={() => setConfirmAiModal({ open: true, ticker: row.ticker })} className="border border-emerald-300 text-emerald-600">אשר</Button>
                            <Button onClick={() => archiveTicker(row.ticker)} className="border border-yellow-300 text-yellow-600">ארכיון</Button>
                            <Button onClick={() => removeTicker(row.ticker)} className="border border-red-300 text-red-600">מחק</Button>
                          </div>
                        </td>
                      </tr>
                    );
        ...
        <div className={`rounded-xl border p-3 text-xs ${theme.card} ${theme.muted}`}>
          Smart Alert: אם המחיר עבר את היעד — השורה תעלה למעלה ותסומן. אם המחיר קרוב עד 3% — תקבל סימון קרוב.
        </div>
      </div>

      {showSettings && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/30 p-4" onClick={() => setShowSettings(false)}>
          <section className={`w-full max-w-6xl rounded-3xl border p-5 shadow-2xl ${theme.card}`} onClick={event => event.stopPropagation()}>
            <div className="mb-5 flex items-center justify-between">
              ...
              <button onClick={() => setShowSettings(false)} className="rounded-full border border-slate-300 bg-white px-4 py-2 text-sm font-black text-slate-700 shadow-sm hover:bg-slate-100">סגור</button>
            </div>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              <div className="rounded-2xl border border-slate-200 bg-white/80 p-3 shadow-sm">
                <div className={`mb-2 text-xs font-black ${theme.muted}`}>גודל כפתורים</div>
                <div className="grid grid-cols-3 gap-2">
                  {[["sm", "קטן"], ["md", "רגיל"], ["lg", "גדול"]].map(([value, label]) => (
                    <button key={value} onClick={() => setButtonSize(value)} className={`rounded-full px-3 py-2 text-xs font-black transition ${buttonSize === value ? "bg-slate-900 text-white shadow-md" : "bg-slate-100 text-slate-700 hover:bg-slate-200"}`}>{label}</button>
                  ))}
                </div>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-white/80 p-3 shadow-sm">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[["none", "שטוח"], ["normal", "עדין"], ["strong", "חזק"]].map(([value, label]) => (
                    <button key={value} onClick={() => setButtonShadow(value)} className={`rounded-full px-3 py-2 text-xs font-black transition ${buttonShadow === value ? "bg-slate-900 text-white shadow-md" : "bg-slate-100 text-slate-700 hover:bg-slate-200"}`}>{label}</button>
                  ))}
                </div>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-white/80 p-3 shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[["rounded-xl", "מעוגל"], ["rounded-full", "עגול"], ["rounded-none", "חד"], ["btn-3d", "תלת ממד"]].map(([value, label]) => (
                    <button key={value} onClick={() => setButtonStyle(value)} className={`rounded-full px-3 py-2 text-xs font-black transition ${buttonStyle === value ? "bg-slate-900 text-white shadow-md" : "bg-slate-100 text-slate-700 hover:bg-slate-200"}`}>{label}</button>
                  ))}
                </div>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-white/80 p-3 shadow-sm">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className={`w-[420px] rounded-2xl border p-4 ${theme.card}`}>
            ...
            <div className="mt-3 flex justify-end gap-2">
              ...
              <button onClick={() => { setRows(prev => prev.map(row => row.ticker === confirmAiModal.ticker ? { ...row, userStatus: row.analysis?.aiStatus || "WATCH" } : row)); setConfirmAiModal({ open: false, ticker: null }); }} className="rounded-lg border border-emerald-300 px-3 py-1 text-sm font-bold text-emerald-700">כן, מאשר</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className={`w-[420px] rounded-2xl border p-4 ${theme.card}`}>
            ...
            <div className="mt-3 flex justify-end gap-2">
              ...
              <button onClick={() => { setRows(prev => prev.filter(row => row.ticker !== confirmDeleteModal.ticker)); setConfirmDeleteModal({ open: false, ticker: null }); }} className="rounded-lg border border-red-300 px-3 py-1 text-sm font-bold text-red-700">כן, מחק</button>
            </div>
          </div>
        </div>
      )}
      ...
      {archiveModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className={`w-[420px] rounded-2xl border p-4 ${theme.card}`}>
            ...
            <textarea value={archiveModal.reason} onChange={e => setArchiveModal(prev => ({ ...prev, reason: e.target.value }))} placeholder="לא חובה אבל מומלץ" className={`mt-3 w-full rounded-xl border p-3 text-sm ${theme.input}`} />
            <div className="mt-3 flex justify-end gap-2">
              <button onClick={() => setArchiveModal({ open: false, ticker: null, reason: "" })} className="rounded-lg border px-3 py-1 text-sm">בטל</button>
              <button onClick={() => { setRows(prev => prev.map(row => row.ticker === archiveModal.ticker ? { ...row, archived: true, archiveReason: archiveModal.reason || "" } : row)); setArchiveModal({ open: false, ticker: null, reason: "" }); }} className="rounded-lg border border-yellow-300 px-3 py-1 text-sm font-bold text-yellow-700">אשר ארכיון</button>
            </div>
          </div>
        </div>
      )}
