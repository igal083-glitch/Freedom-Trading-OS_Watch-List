    ...
    <div dir="rtl" className={`min-h-screen p-4 lg:p-6 ${theme.app}`}>
      ...
      <div className="mx-auto w-full max-w-[96vw] space-y-5">
        ...
        {activePanel === "guide" && (
          <section className={`rounded-3xl border p-5 ${theme.card}`}>
            <div className="mb-3 flex items-center justify-between gap-3">
              <h2 className={`text-xl font-black text-cyan-300`}>מדריך זיהוי Setup + Volume</h2>
        ...
        {activePanel === "archive" && (
          <section className={`rounded-3xl border p-5 ${theme.card}`}>
            <div className="mb-3 flex items-center justify-between gap-3">
              <div><h2 className={`text-xl font-black ${theme.strong}`}>מסך ארכיון</h2><p className={`mt-1 text-sm ${theme.muted}`}>מניות שהועברו לארכיון.</p></div>
              <Button onClick={() => setActivePanel("main")} className={theme.accent}>חזור למסך ראשי</Button>
            </div>
          ...
          <section className={`overflow-hidden rounded-3xl border ${theme.card}`}>
            <div className="border-b border-slate-800/80 bg-[#07111f]/80 px-5 py-4">
              <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
                <div><h2 className={`text-lg font-black ${theme.strong}`}>רשימת מעקב חכמה</h2><p className={`mt-1 text-xs ${theme.muted}`}>לחיצה על כל שורת מניה פותחת מגירה מתחת לשורה.</p></div>
                <div className="flex flex-wrap gap-2 text-xs"><span className="rounded-full border border-slate-700 bg-slate-900 px-3 py-1 text-slate-300">סה״כ: {visibleRows.length}</span><span className="rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1 text-emerald-300">READY: {counts.READY}</span><span className="rounded-full border border-yellow-500/30 bg-yellow-500/10 px-3 py-1 text-yellow-300">WATCH: {counts.WATCH}</span></div>
              </div>
            </div>
                            ...
                            <td colSpan={11} className="p-0">
                              <div className="border-t border-slate-700/60 bg-gradient-to-l from-[#111827] via-[#0b1220] to-[#050816] px-5 py-5">
                                <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                                  <div><div className="flex items-center gap-3"><span className="rounded-2xl border border-slate-600 bg-slate-900 px-4 py-2 text-2xl font-black text-white">{row.ticker}</span><span className={`rounded-full px-3 py-1 text-xs font-black ${priority.className}`}>{priority.text}</span><span className={`rounded-full border px-3 py-1 text-xs font-black ${row.alert.className}`}>{row.alert.label}</span></div><p className={`mt-2 text-xs ${theme.muted}`}>מגירת עבודה — תזה, אלרט, מבנה, אזור כניסה ופסילת טרייד.</p></div>
                                ...
                                <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr_360px]">
                                  <div className="space-y-4">
                                    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                                      <div className={`rounded-2xl border p-4 ${theme.soft} ${theme.border}`}><div className={`text-xs font-black ${theme.muted}`}>Priority</div><div className="mt-2 text-3xl font-black text-white">{row.priority}</div></div>
                                      <div className={`rounded-2xl border p-4 ${theme.soft} ${theme.border}`}><div className={`text-xs font-black ${theme.muted}`}>AI</div><div className="mt-3"><span className={`rounded-xl border px-3 py-2 text-sm font-black ${statusClass(a.aiStatus)}`}>{a.aiStatus}</span></div></div>
                                      <div className={`rounded-2xl border p-4 ${theme.soft} ${theme.border}`}><div className={`text-xs font-black ${theme.muted}`}>Entry</div><div className="mt-2 text-lg font-black text-emerald-300">{a.entryZone}</div></div>
                                      <div className={`rounded-2xl border p-4 ${theme.soft} ${theme.border}`}><div className={`text-xs font-black ${theme.muted}`}>Invalidation</div><div className="mt-2 text-lg font-black text-red-300">{a.invalidation}</div></div>
                                    </div>

                                    <div className={`rounded-3xl border p-4 ${theme.soft} ${theme.border}`}><h4 className={`mb-3 text-base font-black ${theme.strong}`}>מבנה והחלטת טרייד</h4><div className="grid gap-3 lg:grid-cols-3"><div className="rounded-2xl border border-slate-700/70 bg-black/20 p-3"><div className={`text-xs ${theme.muted}`}>Structure</div><div className="mt-2 text-sm font-bold text-slate-100">{a.structure}</div></div><div className="rounded-2xl border border-slate-700/70 bg-black/20 p-3"><div className={`text-xs ${theme.muted}`}>Why</div><div className="mt-2 text-sm text-slate-300">{a.why}</div></div><div className="rounded-2xl border border-slate-700/70 bg-black/20 p-3"><div className={`text-xs ${theme.muted}`}>Score</div><div className="mt-3 flex items-center gap-3"><div className="h-2 flex-1 rounded-full bg-slate-700"><div className="h-2 rounded-full bg-cyan-300" style={{ width: `${a.score}%` }} /></div><b className="text-white">{a.score}</b></div></div></div></div>

                                    <div className={`rounded-3xl border p-4 ${theme.soft} ${theme.border}`}><h4 className={`mb-3 text-base font-black ${theme.strong}`}>תזה אישית</h4><div className="grid gap-3 lg:grid-cols-[180px_1fr]"><div><label className={`mb-2 block text-sm font-black ${theme.muted}`}>תאריך</label><input type="date" value={row.thesisDate || today} onChange={(e) => updateRow(row.ticker, { thesisDate: e.target.value })} className={`w-full rounded-2xl border p-3 text-sm outline-none ${theme.input}`} /></div><div><label className={`mb-2 block text-sm font-black ${theme.muted}`}>כותרת</label><select value={row.thesisTitle || ""} onChange={(e) => updateRow(row.ticker, { thesisTitle: e.target.value })} className={`w-full rounded-2xl border p-3 text-sm outline-none ${theme.input}`}><option value="">בחר תבנית</option><option value="Spike + Wait Base">Spike + Wait Base</option><option value="Pullback Continuation">Pullback Continuation</option><option value="Base Before Breakout">Base Before Breakout</option><option value="Breakout Setup">Breakout Setup</option><option value="Range / Accumulation">Range / Accumulation</option><option value="Weak / Avoid">Weak / Avoid</option></select></div></div><textarea value={row.thesis || ""} onChange={(e) => updateRow(row.ticker, { thesis: e.target.value })} placeholder="מה ראיתי / תוכנית פעולה" className={`mt-3 min-h-[140px] w-full rounded-2xl border p-3 text-sm outline-none ${theme.input}`} /></div>
                                  </div>

                                  <div className="space-y-4">
                                    <div className={`rounded-3xl border p-4 ${theme.soft} ${theme.border}`}><h4 className={`mb-3 text-base font-black ${theme.strong}`}>אלרט ובדיקה</h4><input type="number" value={row.alertPrice || ""} onChange={(e) => updateRow(row.ticker, { alertPrice: e.target.value })} placeholder="Alert Price" className={`w-full rounded-2xl border p-3 text-sm outline-none ${theme.input}`} /><select value={row.alertType || "above"} onChange={(e) => updateRow(row.ticker, { alertType: e.target.value })} className={`mt-3 w-full rounded-2xl border p-3 text-sm outline-none ${theme.input}`}><option value="above">Above — מעל מחיר</option><option value="below">Below — מתחת מחיר</option></select><div className={`mt-3 rounded-2xl border p-3 ${row.alert.className}`}><div className="text-xs font-black">מצב אלרט</div><div className="mt-1 text-lg font-black">{row.alert.label}</div></div><Button onClick={async () => { setLoading(true); setLastError(""); try { const updated = await loadTicker(row); setRows((prev) => prev.map((item) => item.ticker === row.ticker ? updated : item)); setLastRefreshTime(new Date().toLocaleTimeString()); } catch { setLastError("בדיקת מניה נכשלה."); } finally { setLoading(false); } }} className="mt-3 w-full border border-emerald-500/30 bg-emerald-500/10 text-emerald-300">{loading ? "טוען..." : "טען מניה"}</Button></div>
      ...
      {deleteTicker && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className={`w-full max-w-md rounded-3xl border p-6 ${theme.card}`}>
            ...
            <p className={`mt-2 text-sm ${theme.muted}`}>האם למחוק את {deleteTicker} מהרשימה?</p>
            <div className="mt-5 flex gap-3">
              <Button
                onClick={() => {
                  setRows((prev) => prev.filter((row) => row.ticker !== deleteTicker));
      ...
      {archiveModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className={`w-full max-w-lg rounded-3xl border p-6 ${theme.card}`}>
            <h3 className={`text-xl font-black ${theme.strong}`}>
              העבר לארכיון
            </h3>
